...
$sql = "SELECT puntuacion, fecha, errores, id_nivel FROM puntuaciones WHERE id_usuario = ?";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $row['fecha'] = date("d-m-Y", strtotime($row['fecha'])); // Formateo de fecha
        $puntuaciones[] = $row;
    }
} else {
    $puntuaciones = null;
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    <?php foreach ($puntuaciones as $puntuacion): ?>
                        <tr>
                            <td><?= htmlspecialchars($puntuacion['id_nivel']) ?></td>
